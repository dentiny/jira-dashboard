...

{
  ...
  prTasks: `You are addressing GitHub PR tasks for a ${PNAME} ticket. The PR has failing and pending checks that all need attention.

CRITICAL: Do NOT make any code changes. Do NOT edit any files. Only use the \`gh\` CLI.

Read the context file for PR details. Use \`gh\` to resolve ALL checks listed — both failing and pending. For pending checks, investigate what's blocking them and take action (e.g. re-trigger, request reviewers, add labels, comment).`,
